...
def write_run_outputs(output_root: str, slot_results: list[SlotResult], summary: dict[str, object]) -> Path:
    ...
    _write_twin_metrics(run_dir / "twin_metrics.csv", slot_results)
        ...
        for slot_result in slot_results:
            writer.writerow(
                [
                    ...
                    round(slot_result.total_delay, 6),
                    round(slot_result.total_energy, 6),
...
def _write_twin_metrics(path: Path, slot_results: list[SlotResult]) -> None:
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.writer(handle)
        writer.writerow(
            [
        ...
        for slot_result in slot_results:
            for summary in slot_result.local_summaries:
                tier = summary.ap_id.split("_")[0]
                writer.writerow(
                    [
                        slot_result.slot,
                        summary.ap_id,
                        tier,
                        round(summary.fidelity, 6),
                        round(summary.trust, 6),
                        round(summary.mismatch, 6),
                        round(summary.uncertainty, 6),
                        summary.twin_age,
                        int(summary.sync_triggered),
                        int(summary.coordination_triggered),
                        summary.queue_size,
                        round(summary.local_load, 6),
                    ]
                )
